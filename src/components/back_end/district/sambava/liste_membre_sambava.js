...
const ListeMembresSAMBAVA = () =>{
    ...
    const [liste_des_membres_district_sambava, setliste_des_membres_district_sambava] = useState([]);
    const [currentPage, setCurrentPage] = useState(1);
    const recordsPerPage = 10;
    const lastIndex = currentPage * recordsPerPage;
    const firstIndex = lastIndex - recordsPerPage;
    const records = liste_des_membres_district_sambava.slice(firstIndex, lastIndex);
    const npage = Math.ceil(liste_des_membres_district_sambava.length / recordsPerPage);
    ...
    useEffect(() =>{
        axios.get(`api/liste_des_membres_district_sambava`).then(res =>{
            if(res.status === 200){
                setliste_des_membres_district_sambava(res.data.liste_des_membres_district_sambava);   
                }
         });
        fetchUserId();
     },[]);
        ...
        if(data.search == ''){
            ...
            axios.get(`api/recherche_un__membre_fimpisava/${data.select}/${data.search}`).then(res =>{
                ...
                if(res.data.status  === 200){
                    ...
                    setliste_des_membres_district_sambava(res.data.recherche_un__membre_fimpisava);
...
export default ListeMembresSAMBAVA;