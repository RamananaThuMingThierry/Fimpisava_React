...
const ListeMembresVohemar = () =>{
    ...
    const [liste_des_membres_district_vohemar, setliste_des_membres_district_vohemar] = useState([]);
    const [currentPage, setCurrentPage] = useState(1);
    const recordsPerPage = 10;
    const lastIndex = currentPage * recordsPerPage;
    const firstIndex = lastIndex - recordsPerPage;
    const records = liste_des_membres_district_vohemar.slice(firstIndex, lastIndex);
    const npage = Math.ceil(liste_des_membres_district_vohemar.length / recordsPerPage);
    ...
    useEffect(() =>{
        axios.get(`api/liste_des_membres_district_vohemar`).then(res =>{
            if(res.status === 200){
                setliste_des_membres_district_vohemar(res.data.liste_des_membres_district_vohemar);   
                }
         });
        fetchUserId();
     },[]);
        ...
        if(data.search == ''){
            ...
            axios.get(`api/recherche_un__membre_fimpisava/${data.select}/${data.search}`).then(res =>{
                ...
                if(res.data.status  === 200){
                    ...
                    setliste_des_membres_district_vohemar(res.data.recherche_un__membre_fimpisava);
...
export default ListeMembresVohemar;