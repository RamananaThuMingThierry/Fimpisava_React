...
const ListeMembresAntalaha = () =>{
    ...
    const [liste_des_membres_district_antalaha, setliste_des_membres_district_antalaha] = useState([]);
    const [currentPage, setCurrentPage] = useState(1);
    const recordsPerPage = 10;
    const lastIndex = currentPage * recordsPerPage;
    const firstIndex = lastIndex - recordsPerPage;
    const records = liste_des_membres_district_antalaha.slice(firstIndex, lastIndex);
    const npage = Math.ceil(liste_des_membres_district_antalaha.length / recordsPerPage);
    ...
    useEffect(() =>{
        axios.get(`api/liste_des_membres_district_antalaha`).then(res =>{
            if(res.status === 200){
                setliste_des_membres_district_antalaha(res.data.liste_des_membres_district_antalaha);   
                }
         });
        fetchUserId();
     },[]);
        ...
        if(data.search == ''){
            ...
            axios.get(`api/recherche_un__membre_fimpisava/${data.select}/${data.search}`).then(res =>{
                ...
                if(res.data.status  === 200){
                    ...
                    setliste_des_membres_district_antalaha(res.data.recherche_un__membre_fimpisava);
...
export default ListeMembresAntalaha;